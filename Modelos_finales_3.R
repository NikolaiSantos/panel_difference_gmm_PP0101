# -----------------------------
# MODELO 3.1 - MUNICIPALIDADES LIMA-CALLAO
# MODELO 3.2 - MUNICIPALIDADES PROVINCIALES RESTO DEL PAÍS
# MODELO 3.3 - MUNICIPALIDADES PROVINCIALES RESTO DEL PAÍS
# -----------------------------
library(plm)
library(dplyr)
library(readxl)
...
datos_provinciales <- datos_nolmc %>% filter(ES_PROVINCIAL)
datos_distritales   <- datos_nolmc %>% filter(!ES_PROVINCIAL)

cat("Municipalidades distritales de Lima y Callao:", n_distinct(datos_lmc$UBIGEO), "\n")
cat("Municipalidades provinciales resto del país:", n_distinct(datos_provinciales$UBIGEO), "\n")
cat("Municipalidades distritales resto del país:",   n_distinct(datos_distritales$UBIGEO), "\n")
...
modelo_3_2 <- estimar_modelo_gmm(datos_provinciales, "Modelo 3.2 - Municipalidades provinciales del resto del país")
modelo_3_3 <- estimar_modelo_gmm(datos_distritales, "Modelo 3.3 - Municipalidades distritales del resto del país")
...
if (!is.null(modelo_3_1)) {
  coef_2_1 <- graficar_ciclo_electoral(modelo_3_1)
}

if (!is.null(modelo_3_2)) {
  coef_2_1 <- graficar_ciclo_electoral(modelo_3_2)
}

if (!is.null(modelo_3_3)) {
  coef_2_2 <- graficar_ciclo_electoral(modelo_3_3)
}
...
if (!is.null(modelo_3_1) && 
  ...
  cat("Distritales de Lima y Callao:", nrow(modelo_3_1$datos_panel), "\n")
  cat("Provinciales del resto del país:", nrow(modelo_3_2$datos_panel), "\n")
  cat("Distritales del resto del país:", nrow(modelo_3_3$datos_panel), "\n")
  
  cat("\nNúmero de municipalidades:\n")
  cat("Distritales de Lima y Callao:", length(unique(index(modelo_3_1$datos_panel, "UBIGEO"))), "\n")
  cat("Provinciales del resto del país:", length(unique(index(modelo_3_2$datos_panel, "UBIGEO"))), "\n")
  cat("Distritales del resto del país:", length(unique(index(modelo_3_3$datos_panel, "UBIGEO"))), "\n")
}