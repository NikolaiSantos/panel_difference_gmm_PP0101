...
# Definir ruta base
#ruta_base <- "/home/nikolai/Dropbox/Trabajos/TESIS MAESTRIA/Tesis 2/Datasets/RENAMU/2018/"
ruta_base <- "D:/Dropbox/Trabajos/TESIS MAESTRIA/Tesis 2/Datasets/RENAMU/2018"
...
columnas_deseadas <- c("P17_5", "P19", "P20_2_T", "P23_1", "P23_2", "P61A_1", "P61A_2", "P61A_3",
                       "P61A_4", "P61A_5", "P61A_6", "P61A_7", "P61A_8", "P61A_9", "P61A_10", "P61A_11",
                       "P106_4", "P107", "P107_2")

# Buscar y cargar Directorio.csv con separador punto y coma
cdf_dir <- NULL
for (carpeta in carpetas) {
  archivos <- dir_ls(carpeta, regexp = "Directorio\\.csv$")
...
# Verificar si se encontró Directorio.csv
if (is.null(cdf_dir)) {
  stop("No se encontró el archivo Directorio.csv en ninguna carpeta")
}

# Verificar que Directorio.csv tenga la columna idmunici
if (!"idmunici" %in% colnames(cdf_dir)) {
  stop("El archivo Directorio.csv no contiene la columna 'idmunici'")
}

# Inicializar el dataframe resultado con las columnas de Directorio
resultado <- cdf_dir %>% 
...
for (carpeta in carpetas) {
  ...
  for (archivo in archivos_csv) {
    # Saltar el archivo Directorio.csv que ya usamos como base
    if (basename(archivo) == "Directorio.csv") next
...
write_xlsx(resultado, path = file.path(ruta_base, "RENAMU_2018.xlsx"))
write.csv(resultado, file = file.path(ruta_base, "RENAMU_2018.csv"),
...
cat(sprintf("Archivo exportado como: %s\n", file.path(ruta_base, "RENAMU_2018.xlsx")))
